...
import { ReactComponent as RemoveTagIcon } from "./assets/closeIcon.svg";
import ErrorMsg from "../../errorMsg/ErrorMsg";
import styles from "./expandingDropdown.module.scss";
...
export default function ExpandingDropdown<T extends DropdownMenuItem | null>({
  selectedItems,
  placeHolder,
  menuItems,
  isMulti,
    ...
    if (!isMulti) {
      if (selectedItems[0] !== null) {
        ...
        if (
          ...
          window.confirm(
            `Selecting ${item.label} will erase any ${selectedItems[0].label} information you may have entered.`
          ) === true
        ) {
  ...
  return (
    <div>
      ...
      <div className={styles.container}>
        ...
        <div
          ref={menuRef}
          className={`${styles.menu} ${showMenu ? styles.open : styles.closed}`}
        >
          {isSearchable && (
            ...
            if (option && option.id !== "" && option.label !== "") {
              return (
                <button
                  ...
                  key={option.id}
                  className={`${isSelected(option) ? styles.active : ""}`}
                >
                  {option.label}
                </button>
              );
